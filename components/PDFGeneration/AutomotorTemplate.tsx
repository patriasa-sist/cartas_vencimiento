		...
		<BaseTemplate letterData={letterData}>
			...
			<View style={automotorStyles.policyTable}>
				<View style={automotorStyles.tableRow}>
					...
					<View style={[automotorStyles.tableCol, { width: "20%" }]}>
						<Text style={automotorStyles.headerText}>PRIMA TOTAL</Text>
					</View>
				</View>
				...
				{letterData.policies.map((policy, index) => (
					<View key={index} style={automotorStyles.tableRow}>
						...
						<View style={[automotorStyles.tableCol, { width: "20%" }]}>
							<Text style={automotorStyles.cellText}>{formatMonetaryValue(policy.manualFields?.premium, policy.manualFields?.premiumCurrency)}</Text>
						</View>
					</View>
				))}
			</View>
			...
			{letterData.policies.map((policy, policyIndex) => (
				<View key={policyIndex} style={automotorStyles.detailsBox}>
					...
					{policy.manualFields?.vehicles && policy.manualFields.vehicles.length > 0 && (
						<View style={automotorStyles.subTable}>
							<View style={automotorStyles.subTableHeader}>
								...
								<View style={[automotorStyles.subTableColHeader, { width: "25%", borderRightWidth: 0 }]}>
					...
					{policy.manualFields?.deductibles !== undefined && (
						<Text style={automotorStyles.detailText}>• Deducible coaseguro: {formatMonetaryValue(policy.manualFields.deductibles, policy.manualFields.deductiblesCurrency)}</Text>
					)}
					{policy.manualFields?.territoriality !== undefined && (
						<Text style={automotorStyles.detailText}>• Extraterritorialidad (opcional): {formatMonetaryValue(policy.manualFields.territoriality, policy.manualFields.territorialityCurrency)}</Text>
					)}
					{policy.manualFields?.specificConditions && <Text style={automotorStyles.detailText}>• Condiciones Específicas: {policy.manualFields.specificConditions}</Text>}
				</View>
			))}
		</BaseTemplate>
	);
